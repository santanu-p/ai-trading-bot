...
class Settings:
    app_name: str = field(default_factory=lambda: os.getenv("APP_NAME", "AI Trading Bot"))
    ...
    session_secret: str = field(default_factory=lambda: os.getenv("SESSION_SECRET", os.getenv("JWT_SECRET", "change-me")))
    session_expire_minutes: int = field(default_factory=lambda: _env_int("SESSION_EXPIRE_MINUTES", _env_int("JWT_EXPIRE_MINUTES", 720)))
    session_cookie_name: str = field(default_factory=lambda: os.getenv("SESSION_COOKIE_NAME", "tradingbot_session"))
    csrf_cookie_name: str = field(default_factory=lambda: os.getenv("CSRF_COOKIE_NAME", "tradingbot_csrf"))
    ...
    csrf_origin_enforcement: bool = field(default_factory=lambda: _env_bool("CSRF_ORIGIN_ENFORCEMENT", True))
    ...
    web_origin: str = field(default_factory=lambda: os.getenv("WEB_ORIGIN", "http://localhost:3000"))
    ...
    alpaca_api_secret: str | None = field(default_factory=lambda: os.getenv("ALPACA_API_SECRET"))
    alpaca_paper_api_key: str | None = field(default_factory=lambda: os.getenv("ALPACA_PAPER_API_KEY"))
    alpaca_paper_api_secret: str | None = field(default_factory=lambda: os.getenv("ALPACA_PAPER_API_SECRET"))
    alpaca_live_api_key: str | None = field(default_factory=lambda: os.getenv("ALPACA_LIVE_API_KEY"))
    alpaca_live_api_secret: str | None = field(default_factory=lambda: os.getenv("ALPACA_LIVE_API_SECRET"))
    ...
    alpaca_market_data_feed: str = field(default_factory=lambda: os.getenv("ALPACA_MARKET_DATA_FEED", "iex"))
    market_timezone: str = field(default_factory=lambda: os.getenv("MARKET_TIMEZONE", "America/New_York"))
    scan_interval_minutes: int = field(default_factory=lambda: _env_int("SCAN_INTERVAL_MINUTES", 5))
    consensus_threshold: float = field(default_factory=lambda: _env_float("CONSENSUS_THRESHOLD", 0.64))
    min_approval_votes: int = field(default_factory=lambda: _env_int("MIN_APPROVAL_VOTES", 2))
    allow_live_trading: bool = field(default_factory=lambda: _env_bool("ALLOW_LIVE_TRADING", False))
    live_trading_allowed_brokers: tuple[str, ...] = field(default_factory=lambda: _env_csv("LIVE_TRADING_ALLOWED_BROKERS"))
    live_enable_code_ttl_minutes: int = field(default_factory=lambda: _env_int("LIVE_ENABLE_CODE_TTL_MINUTES", 10))
    intraday_flatten_buffer_minutes: int = field(default_factory=lambda: _env_int("INTRADAY_FLATTEN_BUFFER_MINUTES", 15))
    request_body_max_bytes: int = field(default_factory=lambda: _env_int("REQUEST_BODY_MAX_BYTES", 1_000_000))
    api_rate_limit_per_minute: int = field(default_factory=lambda: _env_int("API_RATE_LIMIT_PER_MINUTE", 240))
    auth_rate_limit_per_minute: int = field(default_factory=lambda: _env_int("AUTH_RATE_LIMIT_PER_MINUTE", 20))
    rate_limit_window_seconds: int = field(default_factory=lambda: _env_int("RATE_LIMIT_WINDOW_SECONDS", 60))
    stream_poll_interval_seconds: int = field(default_factory=lambda: _env_int("STREAM_POLL_INTERVAL_SECONDS", 5))
    alert_webhook_urls: tuple[str, ...] = field(default_factory=lambda: _env_csv("ALERT_WEBHOOK_URLS"))
    alert_webhook_timeout_seconds: float = field(default_factory=lambda: _env_float("ALERT_WEBHOOK_TIMEOUT_SECONDS", 5.0))
